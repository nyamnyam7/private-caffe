...
void LibDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i=0; i<num_blobs_; i++) {
    ...
    iface_.get_shape(libuserdata_, i, shape);
    shape[0] = batch_size_;
    top[i]->Reshape(shape);
  }
...
template <typename Dtype>
void LibDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  iface_.reset(libuserdata_, pos_, batch_size_);
  for (int i=0; i<top.size(); i++)
      iface_.fill(libuserdata_, i, pos_, top[i]->shape(), top[i]->mutable_cpu_data());
  pos_ += batch_size_;
}
