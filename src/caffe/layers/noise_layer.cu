...
void NoiseLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->phase_ == TRAIN) {
    ...
    NoiseForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
...
template <typename Dtype>
__global__ void NoiseBackward(const int n, const Dtype* in_diff,
    const unsigned int* mask, const unsigned int threshold, const float scale,
    Dtype* out_diff) {
  CUDA_KERNEL_LOOP(index, n) {
...
void NoiseLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    if (this->phase_ == TRAIN) {
      ...
      const int count = bottom[0]->count();
      // NOLINT_NEXT_LINE(whitespace/operators)
      NoiseBackward<Dtype><<<CAFFE_GET_BLOCKS(count),
